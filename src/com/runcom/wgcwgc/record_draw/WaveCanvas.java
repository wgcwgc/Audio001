package com.runcom.wgcwgc.record_draw;
...
import com.runcom.wgcwgc.record_wavelibrary_utils.Pcm2Wav;
import com.runcom.wgcwgc.record_wavelibrary_view.WaveSurfaceView;

/**
 * 录音和写入文件使用了两个不同的线程，以免造成卡机现象 录音波形绘制
 * 
 * @author cokus
 */
public class WaveCanvas
{
	 ...
	 * @param audioName
	 */
	public void Start(AudioRecord audioRecord , int recBufSize , SurfaceView sfv , String audioName , String path , Callback callback )
	{
	...
	class RecordTask extends AsyncTask < Object , Object , Object >
	{
		...
		public RecordTask(AudioRecord audioRecord , int recBufSize , SurfaceView sfv , Paint mPaint , Callback callback)
		{
			this.audioRecord = audioRecord;
			this.recBufSize = recBufSize;
			this.sfv = sfv;
		...
		void SimpleDraw(ArrayList < Short > buf , int baseLine )
		{
			...
			// sfv.getWidth(),height*0.25f+20, paintLine);//第二根线
			// canvas.drawLine(0, height*0.75f+20,
			// sfv.getWidth(),height*0.75f+20, paintLine);//第3根线
