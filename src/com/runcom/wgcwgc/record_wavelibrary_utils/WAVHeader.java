...
public class WAVHeader
{
	...
	private int mNumBytesPerSample; // number of bytes per sample, all channels
									// included.
	...
	public WAVHeader(int sampleRate , int numChannels , int numSamples)
	{
		...
		mNumBytesPerSample = 2 * mChannels; // assuming 2 bytes per sample (for
											// 1 channel)
		mHeader = null;
		setHeader();
	}
	...
	private void setHeader()
	{
		...
		System.arraycopy(new byte []
		{ 'W', 'A', 'V', 'E' } ,0 ,header ,offset ,4);
		offset += 4;

		// set the fmt chunk
		System.arraycopy(new byte []
		{ 'f', 'm', 't', ' ' } ,0 ,header ,offset ,4);
		offset += 4;
		System.arraycopy(new byte []
		{ 0x10, 0, 0, 0 } ,0 ,header ,offset ,4); // chunk size = 16
		offset += 4;
		System.arraycopy(new byte []
		{ 1, 0 } ,0 ,header ,offset ,2); // format = 1 for PCM
		offset += 2;
		...
		System.arraycopy(new byte []
		{ 0x10, 0 } ,0 ,header ,offset ,2);
		offset += 2;

		// set the beginning of the data chunk
		System.arraycopy(new byte []
		{ 'd', 'a', 't', 'a' } ,0 ,header ,offset ,4);
		offset += 4;
