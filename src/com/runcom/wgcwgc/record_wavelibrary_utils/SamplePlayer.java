...
public class SamplePlayer
{
	...
	public SamplePlayer(ShortBuffer samples , int sampleRate , int channels , int numSamples)
	{
		...
		// make sure minBufferSize can contain at least 1 second of audio (16
		// bits sample).
		...
		// Check when player played all the given data and notify user if
		// mListener is set.
		mAudioTrack.setNotificationMarkerPosition(mNumSamples - 1); // Set the
																	// marker to
																	// the end.
		...
		mPlayThread = new Thread()
		{
			public void run()
			{
				...
				while(mSamples.position() < limit && mKeepPlaying)
				{
					...
					// TODO(nfaralli): use the write method that takes a
					// ByteBuffer as argument.
	...
	public void stop()
	{
		if(isPlaying() || isPaused())
		{
			...
			mAudioTrack.stop(); // Unblock mAudioTrack.write() to avoid
								// deadlocks.
			if(mPlayThread != null)
			{
				try
