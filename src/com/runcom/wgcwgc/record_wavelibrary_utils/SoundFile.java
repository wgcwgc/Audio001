...
public class SoundFile
{
	...
	private int mNumSamples; // total number of samples per channel in audio
	                         // file
	...
	// where sicj is the ith sample of the jth channel (a sample is a signed
	// short)
	// M is the number of channels (e.g. 2 for stereo) and N is the number of
	// samples per channel.

	// Member variables for hack (making it work with old version, until app
	...
	{
		/**
		 * Will be called by the SoundFile class periodically with values
		 * between 0.0 and 1.0. Return true to continue loading the file or
		 * recording the audio, and false to cancel or stop recording.
		 */
	...
	// TODO(nfaralli): what is the real list of supported extensions? Is it
	// device dependent?
	...
	public static SoundFile create(String fileName , ProgressListener progressListener ) throws java.io.FileNotFoundException , IOException , InvalidInputException
	{
	...
	// Should be removed when the app will use directly the samples instead of
	// the frames.
	...
	// A SoundFile object should only be created using the static methods
	// create() and record().
	private SoundFile()
	{
	}
	...
	private void ReadFile(File inputFile ) throws java.io.FileNotFoundException , IOException , InvalidInputException
	{
				...
				if(firstSampleData && format.getString(MediaFormat.KEY_MIME).equals("audio/mp4a-latm") && sample_size == 2)
				{
					...
					else
					{
						...
						if(mProgressListener != null)
						{
							...
							{
								// We are asked to stop reading the file.
								// Returning immediately. The
								// SoundFile object is invalid and should NOT be
								// used afterward!
								extractor.release();
								extractor = null;
								codec.stop();
			...
			if(outputBufferIndex >= 0 && info.size > 0)
			{
				...
				if(outputBufferIndex == MediaCodec.INFO_OUTPUT_BUFFERS_CHANGED)
				{
					...
					if(outputBufferIndex == MediaCodec.INFO_OUTPUT_FORMAT_CHANGED)
					{
						...
						// We could check that codec.getOutputFormat(), which is
		...
		mNumSamples = mDecodedBytes.position() / (mChannels * 2); // One sample
		                                                          // = 2 bytes.
		mDecodedBytes.rewind();
		...
		for(i = 0 ; i < mNumFrames ; i ++ )
		{
			...
			mFrameGains[i] = (int) Math.sqrt(gain); // here gain = sqrt(max
			                                        // value of 1st channel)...
			mFrameLens[i] = frameLens; // totally not accurate...
			mFrameOffsets[i] = (int) (i * (1000 * mAvgBitRate / 8) * // = i *
			                                                         // frameLens
	...
	private void RecordAudio()
	{
		if(mProgressListener == null)
		{
			// A progress listener is mandatory here, as it will let us know
		...
		short [] buffer = new short [1024]; // buffer contains 1 mono frame of
		                                    // 1024 16 bits samples
		...
		AudioRecord audioRecord = new AudioRecord(MediaRecorder.AudioSource.DEFAULT , mSampleRate , AudioFormat.CHANNEL_IN_MONO , AudioFormat.ENCODING_PCM_16BIT , minBufferSize);

		// Allocate memory for 20 seconds first. Reallocate later if more is
		// needed.
		...
		while(true)
		{
			...
			// TODO(nfaralli): maybe use the read method that takes a direct
			// ByteBuffer argument.
			audioRecord.read(buffer ,0 ,buffer.length);
			mDecodedSamples.put(buffer);
			// Let the progress listener know how many seconds have been
			// recorded.
	...
	public void WriteFile(File outputFile , float startTime , float endTime ) throws IOException
	{
		...
		// Some devices have problems reading mono AAC files (e.g. Samsung S3).
		// Making it stereo.
		int numChannels = (mChannels == 1) ? 2 : mChannels;

		String mimeType = "audio/mp4a-latm";
		int bitrate = 64000 * numChannels; // rule of thumb for a good quality:
		                                   // 64kbps per channel.
		...
		// Get an estimation of the encoded data based on the bitrate. Add 10%
		// to it.
		...
		int frame_size = 1024; // number of samples per frame per channel for an
		                       // mp4 (AAC) stream.
		byte buffer[] = new byte [frame_size * numChannels * 2]; // a sample is
		                                                         // coded with a
		                                                         // short.
		mDecodedBytes.position(startOffset);
		numSamples += (2 * frame_size); // Adding 2 frames, Cf. priming frames
		                                // for AAC.
		int tot_num_frames = 1 + (numSamples / frame_size); // first AAC frame =
		                                                    // 2 bytes
		...
		int encodedSamplesSize = 0; // size of the output buffer containing the
		                            // encoded samples.
		...
		while(true)
		{
			...
			if( !done_reading && inputBufferIndex >= 0)
			{
				...
				else
				{
					...
					{
						// Input buffer is smaller than one frame. This should
						// never happen.
						continue;
					}
					// bufferSize is a hack to create a stereo file from a mono
					// stream.
					...
					if(mDecodedBytes.remaining() < bufferSize)
					{
						...
						{
							buffer[i] = 0; // pad with extra 0s to make a full
							               // frame.
						}
			...
			if(outputBufferIndex >= 0 && info.size > 0 && info.presentationTimeUs >= 0)
			{
				...
				{ // Hopefully this should not happen.
					estimatedEncodedSize = (int) (estimatedEncodedSize * 1.2); // Add
					                                                           // 20%.
	...
	// files).
	// buffer contains the PCM data: {sample 1 right, sample 1 left, sample 2
	// right, etc.}
	// The size of a sample is assumed to be 16 bits (for a single channel).
	// When done, buffer will contain {sample 1 left, sample 1 right, sample 2
	// left, etc.}
	private void swapLeftRightChannels(byte [] buffer )
	{
		...
		{ // 2 channels, 2 bytes per sample (for one channel).
			// Invalid buffer size.
			return;
		}
	...
	public void WriteWAVFile(File outputFile , float startTime , float endTime ) throws IOException
	{
		...
		byte buffer[] = new byte [1024 * mChannels * 2]; // Each sample is coded
		                                                 // with a short.
		mDecodedBytes.position(startOffset);
	...
	// Debugging method dumping all the samples in mDecodedSamples in a TSV
	// file.
	...
	// If fileName is null or empty, then the default file name (samples.tsv) is
	// used.
	private void DumpSamples(String fileName )
	{
		...
		// If we can't write to that special path, try just writing directly to
		// the SDCard.
